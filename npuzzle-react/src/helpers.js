...
export function getIndex(row, col) {
  return parseInt(row, 10) *  PUZZLES_COUNT[1] + parseInt(col, 10);
}

// Get the row/col pair from a linear index.
export function getMatrixPosition(index) {
  return {
    row: Math.floor(index /  PUZZLES_COUNT[1]),
    col: index % PUZZLES_COUNT[1],
  };
}
